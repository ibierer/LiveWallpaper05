...
bool Graph::initialize() {
    ...
    mVBState = 0;
    mProgram = createProgram(VERTEX_SHADER, FRAGMENT_SHADER);
    _mProgram = createProgram(_VERTEX_SHADER, _FRAGMENT_SHADER);
    if (!mProgram) return false;
    if (!_mProgram) return false;
    ...
    glVertexAttribPointer(POS_ATTRIB, 3, GL_FLOAT, GL_FALSE, sizeof(Wallpaper::Vertex),
                          (const GLvoid*)offsetof(Wallpaper::Vertex, pos));
