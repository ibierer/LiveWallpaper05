...
namespace _3D{
    ...
    const unsigned int indices[] = {
            0, 1, 2, 3, 6, 7, 4, 5, 0, 1 // 10 indices for GL_TRIANGLE_STRIP
    };

    static const char VERTEX_SHADER[] =
            "#version 300 es\n"
            "layout(location = " STRV(POSITION_ATTRIBUTE_LOCATION) ") in vec2 pos;\n"
    ...
    void draw(){
        ...
        glDrawElements(GL_TRIANGLE_STRIP, 12, GL_UNSIGNED_INT, indices);
    }
}
...
void Renderer::step() {
    ...
    if (mLastFrameNs > 0) {
        ...
        float* transforms = mapTransformBuf();
        for (unsigned int i = 0; i < mNumInstances/4; i++) {
            float s = sinf(mAngles[i]);
            float c = cosf(mAngles[i]);
            transforms[4 * i + 0] = c * mScale[0];
            transforms[4 * i + 1] = s * mScale[1];
