...
public:
    ...
    void calculateSurfaceOnCPU(float (*fOfXYZ)(ImplicitGrapher& graph, vec3, const float&, const vec3&, const float&), float timeVariable, uint iterations, vec3 offset, const float& zoom, bool vectorPointsPositive, bool clipEdges, Vertex* vertices, uvec3* _indices, GLuint& numIndices);
};
...
ImplicitGrapher::ImplicitGrapher(ivec3 radius) {
    ...
    vertices = (Vertex*)malloc(maxSolutionCount * sizeof(Vertex));
...
void ImplicitGrapher::calculateSurfaceOnCPU(float (*fOfXYZ)(ImplicitGrapher &graph, vec3, const float&, const vec3&, const float&), float timeVariable, uint iterations, vec3 offset, const float& zoom, bool vectorPointsPositive, bool clipEdges, Vertex* verts, uvec3* _indices, GLuint& numIndices) {
    t = timeVariable;
...
class ImplicitGrapher {
    ...
    bool hasTimeVariable[maxNumOfEquations];
...
vec3 ImplicitGrapher::getCurrentOffset() {
...
bool ImplicitGrapher::anOperator(char character) {
    return
            character == '^' ||
            character == '/' ||
            character == '*' ||
...
uint ImplicitGrapher::getIterations() {
    return iterations;
}

int ImplicitGrapher::node0() {
    switch (_l) {
        case 0: return getXYZLineIndex(_i - 1, _j, _k - 1)[0]; break;//XY-plane[Yz]
        case 1: return getXYZLineIndex(_i - 1, _j, _k)[0]; break;//XY-plane[YZ]
        case 2: return getXYZLineIndex(_i - 1, _j - 1, _k)[1]; break;//YZ-plane[xZ]
        case 3: return getXYZLineIndex(_i, _j - 1, _k)[1]; break;//YZ-plane[XZ]
        case 4: return getXYZLineIndex(_i, _j - 1, _k - 1)[2]; break;//ZX-plane[Xy]
        case 5: return getXYZLineIndex(_i, _j, _k - 1)[2]; break;//ZX-plane[XY]
        default: return 0;
    }
}

int ImplicitGrapher::node1() {
    switch (_l) {
        case 0: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[1]; break;//XY-plane[xz]
        case 1: return getXYZLineIndex(_i - 1, _j - 1, _k)[1]; break;//XY-plane[xZ]
        case 2: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[2]; break;//YZ-plane[xy]
        case 3: return getXYZLineIndex(_i, _j - 1, _k - 1)[2]; break;//YZ-plane[Xy]
        case 4: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[0]; break;//ZX-plane[yz]
        case 5: return getXYZLineIndex(_i - 1, _j, _k - 1)[0]; break;//ZX-plane[Yz]
        default: return 0;
    }
}

int ImplicitGrapher::node2() {
    switch (_l) {
        case 0: return getXYZLineIndex(_i, _j - 1, _k - 1)[1]; break;//XY-plane[Xz]
        case 1: return getXYZLineIndex(_i, _j - 1, _k)[1]; break;//XY-plane[XZ]
        case 2: return getXYZLineIndex(_i - 1, _j, _k - 1)[2]; break;//YZ-plane[xY]
        case 3: return getXYZLineIndex(_i, _j, _k - 1)[2]; break;//YZ-plane[XY]
        case 4: return getXYZLineIndex(_i - 1, _j - 1, _k)[0]; break;//ZX-plane[yZ]
        case 5: return getXYZLineIndex(_i - 1, _j, _k)[0]; break;//ZX-plane[YZ]
        default: return 0;
    }
}

int ImplicitGrapher::node3() {
    switch (_l) {
        case 0: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[0]; break;//XY-plane[yz]
        case 1: return getXYZLineIndex(_i - 1, _j - 1, _k)[0]; break;//XY-plane[yZ]
        case 2: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[1]; break;//YZ-plane[xz]
        case 3: return getXYZLineIndex(_i, _j - 1, _k - 1)[1]; break;//YZ-plane[Xz]
        case 4: return getXYZLineIndex(_i - 1, _j - 1, _k - 1)[2]; break;//ZX-plane[xy]
        case 5: return getXYZLineIndex(_i - 1, _j, _k - 1)[2]; break;//ZX-plane[xY]
        default: return 0;
    }
}
...
float ImplicitGrapher::fOfXYZ(ImplicitGrapher &graph, vec3 _, const float &t, const vec3 &offset, const float &zoom) {
    ...
    // A better solution to below may be near the bottom of the following web page: https://www.cplusplus.com/doc/tutorial/pointers/
    for (int i = 0; i < graph.sequenceLengths[graph.surfaceEquation]; i++) {
        switch (graph.sequences[graph.surfaceEquation][i][0]) {
            case _ADD: graph.values[graph.sequences[graph.surfaceEquation][i][1]] += graph.values[graph.sequences[graph.surfaceEquation][i][2]]; break;
            case _SUBTRACT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] -= graph.values[graph.sequences[graph.surfaceEquation][i][2]]; break;
            case _MULTIPLY: graph.values[graph.sequences[graph.surfaceEquation][i][1]] *= graph.values[graph.sequences[graph.surfaceEquation][i][2]]; break;
            case _DIVIDE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] /= graph.values[graph.sequences[graph.surfaceEquation][i][2]]; break;
            case _POWER: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = powf(graph.values[graph.sequences[graph.surfaceEquation][i][1]], graph.values[graph.sequences[graph.surfaceEquation][i][2]]); break;
            case _SINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = sinf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _COSINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = cosf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _TANGENT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = tanf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _ARC_SINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = asinf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _ARC_COSINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = acosf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _ARC_TANGENT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = atanf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_SINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = sinhf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_COSINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = coshf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_TANGENT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = tanhf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_ARC_SINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = asinhf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_ARC_COSINE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = acoshf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _HYPERBOLIC_ARC_TANGENT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = atanhf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _ABSOLUTE_VALUE: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = abs(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _LOG: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = logf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _NATURAL_LOG: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = (logf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]) / base10LogOfE); break;
            case _SQUARE_ROOT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = sqrtf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
            case _CUBED_ROOT: graph.values[graph.sequences[graph.surfaceEquation][i][1]] = cbrtf(graph.values[graph.sequences[graph.surfaceEquation][i][1]]); break;
        }
    }
...
void ImplicitGrapher::calculateSurfaceOnCPU(float (*fOfXYZ)(ImplicitGrapher &graph, vec3, const float&, const vec3&, const float&), float timeVariable, uint iterations, vec3 offset, const float& zoom, bool vectorPointsPositive, bool clipEdges, VertexNormal* verts, uvec3* _indices, GLuint& numIndices) {
    ...
    gSC = 0;
    // Calculate plusMinusAmounts and approximate solutions
    for (_i = 0; _i < sizePlus3.x; _i++) {
        for (_j = 0; _j < sizePlus3.y; _j++) {
            for (_k = 0; _k < sizePlus3.z; _k++) {
                plusMinus[(_i * sizePlus3.y + _j) * sizePlus3.z + _k] = fOfXYZ(*this, vec3(_i, _j, _k), t, currentOffset, zoom) > 0.0f;
                uvec3 ijk(_i, _j, _k);
                for (_l = 0; _l < 3; _l++) {
                    if (ijk[_l] == 0) {
                    ...
                    int sign = secondSample ? POSITIVE : NEGATIVE;
                    for (int i = 0; i < 3; i++) {
                        verts[sC].v[i] = iCursor[i];
                    }
                    float scan = 0.5f;
                    for (int i = 0; i < iterations; i++) {// Maybe use a do-while loop here to reduce the scan operations by 1.
                        verts[sC].v[_l] += scan;
                        if (sign == POSITIVE ^ fOfXYZ(*this, verts[sC].v, t, currentOffset, zoom) > 0.0f) {
                            scan *= 0.5f;
                        }else{
                ...
                for (_l = 0; _l < 6; _l++) {
                    ...
                    switch (combo) {
                        case 0: break;//0000
                        case 1: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node3(), node2(), -1);
                            else gS[gSC++] = ivec3(node2(), node3(), -1); break;//0001
                        case 2: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node1(), node3(), -1);
                            else gS[gSC++] = ivec3(node3(), node1(), -1); break;//0010
                        case 3: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node1(), node2(), -1);
                            else gS[gSC++] = ivec3(node2(), node1(), -1); break;//0011
                        case 4: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node2(), node0(), -1);
                            else gS[gSC++] = ivec3(node0(), node2(), -1); break;//0100
                        case 5: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node3(), node0(), -1);
                            else gS[gSC++] = ivec3(node0(), node3(), -1); break;//0101
                        case 6: {//0110
                            int _node0 = node0(), _node1 = node1(), _node2 = node2(), _node3 = node3();
                            if (fOfXYZ(*this, (verts[_node0].v + verts[_node1].v + verts[_node2].v + verts[_node3].v) * 0.25f, t, currentOffset, zoom) > 0.0f) {
                                if (_l % 2 != vectorPointsPositive) { gS[gSC++] = ivec3(_node1, _node0, -1); gS[gSC++] = ivec3(_node2, _node3, -1); }
                                else { gS[gSC++] = ivec3(_node0, _node1, -1); gS[gSC++] = ivec3(_node3, _node2, -1); }
                            }
                            ...
                            break; }
                        case 7: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node1(), node0(), -1);
                            else gS[gSC++] = ivec3(node0(), node1(), -1); break;//0111
                        case 8: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node0(), node1(), -1);
                            else gS[gSC++] = ivec3(node1(), node0(), -1); break;//1000
                        case 9: {//1001
                            int _node0 = node0(), _node1 = node1(), _node2 = node2(), _node3 = node3();
                            if (fOfXYZ(*this, (verts[_node0].v + verts[_node1].v + verts[_node2].v + verts[_node3].v) * 0.25f, t, currentOffset, zoom) > 0.0f) {
                                if (_l % 2 != vectorPointsPositive) { gS[gSC++] = ivec3(_node0, _node2, -1); gS[gSC++] = ivec3(_node3, _node1, -1); }
                            ...
                            break; }
                        case 10: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node0(), node3(), -1);
                            else gS[gSC++] = ivec3(node3(), node0(), -1); break;//1010
                        case 11: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node0(), node2(), -1);
                            else gS[gSC++] = ivec3(node2(), node0(), -1); break;//1011
                        case 12: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node2(), node1(), -1);
                            else gS[gSC++] = ivec3(node1(), node2(), -1); break;//1100
                        case 13: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node3(), node1(), -1);
                            else gS[gSC++] = ivec3(node1(), node3(), -1); break;//1101
                        case 14: if (_l % 2 != vectorPointsPositive) gS[gSC++] = ivec3(node2(), node3(), -1);
                            else gS[gSC++] = ivec3(node3(), node2(), -1); break;//1110
                        case 15: break;//1111
                    }
                }
                //Grouping
                ...
                int segPerGroup = 1;
                if (clipEdges) {
                    for (int __i = groupSegmentStartIndex; __i < gSC; __i++) {
                        withinGraphRadius[__i] = _i > 1 && _j > 1 && _k > 1 && _i < sizePlus2.x && _j < sizePlus2.y && _k < sizePlus2.z;
                    }
                }
                for (int __i = groupSegmentStartIndex; __i < gSC - 1; __i++) {
                    for (int __j = __i + 1; __j < gSC; __j++) {
                        if (gS[__j][0] != gS[__i][1]) {
                            continue;
                        }
                        //Swap
                        int storeA = gS[__i + 1][0];
                        int storeB = gS[__i + 1][1];
                        gS[__i + 1][0] = gS[__j][0];
                        gS[__i + 1][1] = gS[__j][1];
                        gS[__j][0] = storeA;
                        gS[__j][1] = storeB;
                        segPerGroup++;
                        //Check to see if end of loop
                        if (gS[__i + 1][1] != gS[__i + 2 - segPerGroup][0]) {
                            continue;
                        }
                        if (segPerGroup == 3) {
                            gS[__i - 1][2] = gS[__i][1];
                            ...
                            vec3 sum(0.0f);
                            for (int __k = __i - segPerGroup + 2; __k < __i + 2; __k++) {
                                sum += verts[gS[__k][0]].v;
                                gS[__k][2] = sC;
                            }
                            verts[sC].v = sum / segPerGroup;
                            sC++;
...
bool Graph::initialize() {
    mProgram = createProgram(_VERTEX_SHADER, _FRAGMENT_SHADER);
    return true;
}
...
void Graph::render(){
    ...
    glVertexAttribPointer(POSITION_ATTRIBUTE_LOCATION, 3, GL_FLOAT, GL_FALSE, sizeof(VertexNormal), (const GLvoid*)&_graph.vertices[0].v);
    glVertexAttribPointer(NORMAL_ATTRIBUTE_LOCATION, 3, GL_FLOAT, GL_FALSE, sizeof(VertexNormal), (const GLvoid*)&_graph.vertices[0].n);
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, _graph.indices);
    glDisableVertexAttribArray(POSITION_ATTRIBUTE_LOCATION);
