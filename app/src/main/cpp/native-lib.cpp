...
void setObstacle(float x, float y, float z, bool reset);  // Updated function signature
...
public:
    ...
    PicFlip(float _density, float _width, float _height, float _depth, float _spacing, float _particleRadius, int _maxParticles);
...
public:
    ...
    void simulate(vec3 acceleration){
        fluid->simulate(
    ...
    void setupScene(){
        ...
        float tankDepth = 1.0 * simDepth;
        float h = tankHeight / res;
        float density = 1000.0;

        float relWaterHeight = 1.0;
        float relWaterWidth = 0.6;
        float relWaterDepth = 1.0;
...
PicFlip::PicFlip(float _density, float _width, float _height, float _depth, float _spacing, float _particleRadius, int _maxParticles) {
...
void PicFlip::integrateParticles(float _dt, vec3 _gravity)
{
    for (int i = 0; i < numParticles; i++) {
        particleVel[i] += _dt * _gravity;
        particlePos[i] += particleVel[i] * _dt;
...
void PicFlip::pushParticlesApart(float _numIters)
{
    float colorDiffusionCoeff = 0.001;
...
void PicFlip::handleParticleCollisions()
{
    float r = particleRadius;
...
void PicFlip::updateParticleDensity()
{
    int n = fNumY;
    float h1 = fInvSpacing;
    float h2 = 0.5 * h;
...
void PicFlip::transferVelocities(bool _toGrid, float _flipRatio)
{
    int n = fNumY;
    float h1 = fInvSpacing;
    float h2 = 0.5f * h;
...
void PicFlip::solveIncompressibility(int _numIters, float _dt, float _overRelaxation, bool _compensateDrift = true) {
    for (int i = 0; i < fNumCells; i++) {
        p[i] = 0.0;
        prevU[i] = u[i];
        prevV[i] = v[i];
...
void PicFlip::updateParticleColors()
{
    float h1 = fInvSpacing;
...
void PicFlip::simulate(float _dt, vec3 _gravity, float _flipRatio, int _numPressureIters, int _numParticleIters, float _overRelaxation, bool _compensateDrift, bool _separateParticles)
{
    int numSubSteps = 1;
    float sdt = _dt / numSubSteps;
