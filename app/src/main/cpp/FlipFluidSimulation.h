...
public:
    ...
    string computeShaderCode[1000] = {
            ...
            //"            vec3 multipliedPosition = position * pInvSpacing;\n",
            //"            uint xi = uint(clamp(floor(multipliedPosition.x), 0.0f, float(pNumX - 1u)));\n",
            //"            uint yi = uint(clamp(floor(multipliedPosition.y), 0.0f, float(pNumY - 1u)));\n",
            //"            uint zi = uint(clamp(floor(multipliedPosition.z), 0.0f, float(pNumZ - 1u)));\n",
            //"            uint cellNr = (xi * pNumY + yi) * pNumZ + zi;\n",
            //"            uint index = outBuffer.pCells[cellNr].numCellParticles;\n",
            //"            outBuffer.pCells[cellNr].positions[index] = position;\n",
            //"            outBuffer.pCells[cellNr].indices[index] = i;\n",
            ...
            "        uint index = atomicAdd(outBuffer.pCells[cellNr].numCellParticles, 1u);\n",
            "        \n",
            "        // Now safe to write to the reserved slot\n",
            "        outBuffer.pCells[cellNr].positions[index] = position;\n",
            "        outBuffer.pCells[cellNr].indices[index] = i;\n",
            "    }\n",
            "    \n",
...
// Serial stable version:
            "    if(task == 0u) {\n",
            "        for (int iter = 0; iter < numIters; iter++) {\n",
            "            \n",
            "            const uint regionSize = 4u;\n",
            "            const uint regionsX = (pNumX + regionSize - 1u) / regionSize;\n",
            "            const uint regionsY = (pNumY + regionSize - 1u) / regionSize;\n",
            "            const uint regionsZ = (pNumZ + regionSize - 1u) / regionSize;\n",
            "            for (uint i = 0u; i <= regionsX; i++) {\n",
            "                for (uint j = 0u; j <= regionsY; j++) {\n",
            "                    for (uint k = 0u; k <= regionsZ; k++) {\n",
            "                        for (uint l = 0u; l < regionSize; l++) {\n",
            "                            uint i_plus_l = regionSize * i + l;\n",
            "                            if(i_plus_l >= pNumX) break;\n",
            "                            for (uint m = 0u; m < regionSize; m++) {\n",
            "                                uint j_plus_m = regionSize * j + m;\n",
            "                                if(j_plus_m >= pNumY) break;\n",
            "                                for (uint n = 0u; n < regionSize; n++) {\n",
            "                                    uint k_plus_n = regionSize * k + n;\n",
            "                                    if(k_plus_n >= pNumZ) break;\n",
            "                                    uint homeCell = (i_plus_l * pNumY + j_plus_m) * pNumZ + k_plus_n;\n",
            "                                    for (uint i = 0u; i < outBuffer.pCells[homeCell].numCellParticles; i++) {\n",
            "                                        vec3 pxyz = outBuffer.pCells[homeCell].positions[i];\n",
            "                                        uint pxi = uint(floor(pxyz.x * pInvSpacing));\n",
            "                                        uint pyi = uint(floor(pxyz.y * pInvSpacing));\n",
            "                                        uint pzi = uint(floor(pxyz.z * pInvSpacing));\n",
            ...
            "                                                    uint neighborCell = (xi * pNumY + yi) * pNumZ + zi;\n",
            "                                                    uint last = outBuffer.pCells[neighborCell].numCellParticles;\n",
            "                                                    for (uint j = 0u; j < last; j++) {\n",
            "                                                        uint id = outBuffer.pCells[neighborCell].indices[j];\n",
            "                                                        if (id == outBuffer.pCells[homeCell].indices[i])\n",
            "                                                            continue;\n",
            "                                                        vec3 dxyz = outBuffer.pCells[neighborCell].positions[j] - pxyz;\n",
            ...
            "                                                        dxyz *= _s;\n",
            "                                                        outBuffer.pCells[homeCell].positions[i] -= dxyz;\n",
            "                                                        outBuffer.pCells[neighborCell].positions[j] += dxyz;\n",
            "                                                    }\n",
            "                                                }\n",
            "                                            }\n",
            "                                        }\n",
...
// Parallel unstable version:
            ...
            "                for (uint n = 0u; n < regionSize; n++) {\n",
            "                    uint k_plus_n = regionSize * k + n;\n",
            "                    bool validCell = i_plus_l < pNumX && j_plus_m < pNumY && k_plus_n < pNumZ;\n",
            "                    uint homeCell = (i_plus_l * pNumY + j_plus_m) * pNumZ + k_plus_n;\n",
            "                    for (uint i = 0u; i < 7u; i++) {\n",
            "                        bool validIndex = validCell && (i < outBuffer.pCells[homeCell].numCellParticles);\n",
            "                        vec3 pxyz = (validIndex ? outBuffer.pCells[homeCell].positions[i] : vec3(0.0f));\n",
            "                        uint pxi = uint(floor(pxyz.x * pInvSpacing));\n",
            "                        uint pyi = uint(floor(pxyz.y * pInvSpacing));\n",
            "                        uint pzi = uint(floor(pxyz.z * pInvSpacing));\n",
            "                        uint x0 = max(pxi - 1u, 0u);\n",
            ...
            "                            uint xi = x0 + o;\n",
            "                            bool validX = validIndex && xi <= x1;\n",
            "                            for (uint p = 0u; p <= 3u; p++) {\n",
            "                                uint yi = y0 + p;\n",
            "                                bool validY = validX && yi <= y1;\n",
            "                                for (uint q = 0u; q <= 3u; q++) {\n",
            "                                    uint zi = z0 + q;\n",
            "                                    bool validZ = validY && zi <= z1;\n",
            "                                    uint neighborCell = (xi * pNumY + yi) * pNumZ + zi;\n",
            "                                    uint last = (validZ ? outBuffer.pCells[neighborCell].numCellParticles : 0u);\n",
            "                                    for (uint j = 0u; j < last; j++) {\n",
            "                                        uint id = outBuffer.pCells[neighborCell].indices[j];\n",
            "                                        if (id == outBuffer.pCells[homeCell].indices[i])\n",
            "                                            continue;\n",
            ...
            //"    for (uint i = task; i < pNumCells; i += numGlobalInvocations) {\n",
            "    for (uint i = ceilOfPCellsPerInvocation * task; i < ceilOfPCellsPerInvocation * (task + 1u) && i < pNumCells; i++) {\n",
            "        for (uint j = 0u; j < outBuffer.pCells[i].numCellParticles; j++) {\n",
            "            outBuffer.particles[outBuffer.pCells[i].indices[j]].position = outBuffer.pCells[i].positions[j];\n",
            "        }\n",
            "    }\n",
            "}\n",
            "// Iterate over particles\n",
            ...
            //"    for (uint i = task; i < fNumCells; i += numGlobalInvocations) {\n",
            "    for (uint i = ceilOfFCellsPerInvocation * task; i < ceilOfFCellsPerInvocation * (task + 1u) && i < fNumCells; i++) {\n",
            "        for (uint j = 0u; j < outBuffer.fCells[i].densityContributionsCounter; j++) {\n",
            ...
            //"            outBuffer.logValues[task][j] = 0u;\n",
            //"        }\n",
            "        barrier();\n",
            //"        outBuffer.logValues[task][0] = gl_WorkGroupID.x;\n",
            //"        outBuffer.logValues[task][1] = gl_WorkGroupID.y;\n",
            //"        outBuffer.logValues[task][2] = gl_WorkGroupID.z;\n",
            //"        outBuffer.logValues[task][3] = gl_LocalInvocationID.x;\n",
            //"        outBuffer.logValues[task][4] = gl_LocalInvocationID.y;\n",
            //"        outBuffer.logValues[task][5] = gl_LocalInvocationID.z;\n",
            //"        atomicAdd(outBuffer.logValues[task][6], 1u);\n",
