...
public:
    ...
    string computeShaderCode[1000] = {
            ...
            "                            uint iPlusL = regionSize * i + l;\n",
            "                            if(iPlusL >= pNumX) break;\n",
            "                            for (uint m = 0u; m < regionSize; m++) {\n",
            "                                uint jPlusM = regionSize * j + m;\n",
            "                                if(jPlusM >= pNumY) break;\n",
            "                                for (uint n = 0u; n < regionSize; n++) {\n",
            "                                    uint kPlusN = regionSize * k + n;\n",
            "                                    if(kPlusN >= pNumZ) break;\n",
            "                                    uint homeCell = (iPlusL * pNumY + jPlusM) * pNumZ + kPlusN;\n",
            "                                    for (uint index = 0u; index < outBuffer.pCells[homeCell].numCellParticles; index++) {\n",
            "                                        uint i = outBuffer.pCells[homeCell].indices[index];\n",
