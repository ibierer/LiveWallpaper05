...
public:
    ...
    void calculateSurfaceOnCPU(std::function<float(vec3 _, NaiveSimulation& sim, const vec3& offset, const vec3& defaultOffset)> fOfXYZ, const float &timeVariable, const uint &iterations, const vec3 &offset, const float &zoom, const bool &clipEdges, PositionXYZNormalXYZ *_vertices, uvec3 *_indices, GLuint &_numIndices, NaiveSimulation& sim);
